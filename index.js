...
function addGamesToPage(games) {
    ...
    for (let i=0; i<games.length; i++){
        ...
        game_card.id = game.name;

        // set the inner HTML using a template literal to display some info 
        // about each game
        // TIP: if your images are not displaying, make sure there is space
        // between the end of the src attribute and the end of the tag ("/>")
        game_card.innerHTML = `<img class="game-img" src="${game.img}" alt="${game.name}"/> 
        <h3>${game.name}</h3> 
...
let totalRaised = GAMES_JSON.reduce9((sum, game) => sum + game.pledged, 0);
...
const unfundedGamesText = `A total ${totalRaised.toLocaleString()} has been raised for ${numOfGames} games.
    Currently, ${unfundedGames} games${unfundedGames > 1 ? 's remain' : ' remains'} unfunded.
